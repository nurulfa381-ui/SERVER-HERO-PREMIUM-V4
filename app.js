...
if((state.completedKP||[]).includes(12)&&(state.unlocked||0)<13){state.unlocked=13;save();}
...
<section class="bottom"><article class="card badges"><h2>${t("badges")}</h2><div class="badge-grid">${B.map(b=>`<article class="${state.badges.includes(b.id)?"earned":"locked"}"><span>${b.icon}</span><small>${b[state.lang]}</small></article>`).join("")}</div></article><article class="card achievements"><h2>${t("achievements")}</h2><div><span>📘</span><b>${state.completedKP.length}/13 KP</b></div><div><span>✅</span><b>${pass()}/13 KT</b></div><div><span>🏆</span><b>${rank()}</b></div></article></section>
</div>`;
}
...
if(id===13){window.location.href="kp13.html";return}
