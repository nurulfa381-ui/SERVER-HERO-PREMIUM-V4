...
<article class="card overview"><h1>${t("dashboard")}</h1><p>${t("welcome")}</p><div class="stats"><article><span>${t("progress")}</span><b>${progress()}%</b></article><article><span>${t("average")}</span><b>${avg()}%</b></article><article><span>${t("level")}</span><b>${level()}</b></article><article><span>${t("xp")}</span><b>${state.xp}</b></article></div><div class="next"><div><small>${t("next")}</small><h3>KP${String(next.id).padStart(2,"0")} · ${next[state.lang]}</h3></div><button onclick="openMission(${next.id})">${t("start")} →</button></div></article></section>
